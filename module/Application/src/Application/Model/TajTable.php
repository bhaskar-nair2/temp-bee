...
class TajTable extends AbstractTableGateway {
	...
	public function addTajTargetDetails($params){
		if (trim($params['targetMonth'])!="") {
			...
			$eventType = 'taj target-add';
			$action = 'added a new taj target with the date '."01-".$params['targetMonth'];
			$resourceName = 'Taj';
	...
	public function updateTajTargetDetails($params) {
        if (trim($params['tajId'])!="") {
			$commonService=new CommonService();
			$logincontainer = new Container('credo');
			$tajId=base64_decode($params['tajId']);
			$targetMonth="";
			$targetYear="";
			if(isset($params['targetMonth']) && trim($params['targetMonth'])!=""){
				$targetDate=$commonService->dateFormat("01-".$params['targetMonth']);
            ...
            $this->update($data, array('taj_id' => $tajId));
            //event log
			$subject = $tajId;
			$eventType = 'taj target-update';
			$action = 'updated a taj target with the date '."01-".$params['targetMonth'];
			$resourceName = 'Taj';
			$eventLogDb = new EventLogTable($this->adapter);
			$eventLogDb->addEventLog($subject, $eventType, $action, $resourceName);
			return $tajId;
    ...
    public function fetchAllTajTargetDetails($parameters,$acl) {
        ...
        */
        ...
        foreach ($rResult as $aRow) {
			...
			if($update){
				$row[] = '<a href="./edit/' . base64_encode($aRow['taj_id']) . '" title="Edit"><i class="fa fa-pencil"> </i></a>';
			}
            $output['aaData'][] = $row;
        }
        return $output;
	...
	public function fetchTajTarget($targetId) {
        $row = $this->select(array('taj_id' => (int) $targetId))->current();
        return $row;
    }
    
	public function updateCurrentTarget($corporateId,$currentTarget){
		$this->update(array('current_target'=>$currentTarget), array('taj_id' => $corporateId));
	}
}
?>
