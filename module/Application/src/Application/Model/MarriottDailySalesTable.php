...
class MarriottDailySalesTable extends AbstractTableGateway {

    protected $table = 'marriott_daily_sales';

    public function __construct(Adapter $adapter) {
        $this->adapter = $adapter;
    }
    
	public function addMarriottDailySalesDetails($params) {
        $result = "";
        ...
        if (trim($params['salesDate']) != "") {
            ...
            $data = array(
                ...
                'bth_sales' => $params['bthSale'],
                'btr_sales' => $params['btrSale'],
                'cash' => $params['cash'],
                'total_sales' => $params['totalSales'],
                'non_commisionable' => $params['nonCommisionable'],
			...
			if($corporateId>0){
				...
				$dbAdapter = $this->adapter;
				$marriottDb=new MarriottTable($dbAdapter);
				$marriottDb->updateCurrentTarget($corporateId,$params['monthlyTarget']);
			}
			...
			$cQuery = $sql->select()->from('marriott_daily_sales')
			...
			foreach($cResult as $val){
				$totalSales=$val['bth_sales']+$val['btr_sales']+$val['cash']+$val['non_commisionable'];
				...
				$this->updateMarriottDailySalesCurrentTarget($val['sales_id'],$todayTarget,$currentTarget,$previousTarget,$corporateId);
		...
		$eventType = 'marriott-add';
		$action = 'added a new marriott daily sales with the date '.$params['salesDate'];
	...
	public function updateMarriottDailySalesDetails($params) {
		$dbAdapter = $this->adapter;
		$sql = new Sql($dbAdapter);
        if (trim($params['marriottId'])!="" && trim($params['salesDate']) != "") {
			$commonService=new CommonService();
			$logincontainer = new Container('credo');
            ...
            $this->update($data, array('sales_id' => $marriottId));
			$corporateId=base64_decode($params['corporateId']);
			...
			//event log
			$subject = $marriottId;
			$eventType = 'marriott-update';
			$action = 'updated a marriott daily sales with the date '.$params['salesDate'];
			$resourceName = 'Marriott';
			$eventLogDb = new EventLogTable($this->adapter);
			$eventLogDb->addEventLog($subject, $eventType, $action, $resourceName);
			return $marriottId;
	...
	public function updateMarriottDailySalesCurrentTarget($salesId,$todayTarget,$currentTarget,$previousTarget,$corporateId){
		$data = array(
		...
		if($corporateId>0){
			...
			$dbAdapter = $this->adapter;
			$marriottDb=new MarriottTable($dbAdapter);
			$marriottDb->updateCurrentTarget($corporateId,$currentTarget);
		}
	}
	
    public function fetchAllDailySales($parameters,$acl) {
        /* Array of database columns which should be read and sent back to DataTables. Use a space where
         * you want to insert a non-database field (for example a counter or static image)
        */
		$queryContainer = new Container('query');
        $aColumns = array('DATE_FORMAT(sales_date,"%d-%b-%Y")','bth_sales','btr_sales','cash','non_commisionable','total_sales','daily_target','monthly_target');
        $orderColumns = array('sales_date','bth_sales','btr_sales','cash','non_commisionable','total_sales','daily_target','monthly_target');
        ...
        */
        ...
        $dbAdapter = $this->adapter;
        $sql = new Sql($dbAdapter);
        $sQuery = $sql->select()->from('marriott_daily_sales');
		...
		$iQuery = $sql->select()->from('marriott_daily_sales');
		if(isset($parameters['startDate']) && trim($parameters['startDate'])!="" && isset($parameters['endDate']) && trim($parameters['endDate'])!=""){
        ...
        if ($acl->isAllowed($role, 'Admin\Controller\Marriott', 'edit-daily-sales')) {
            $update = true;
        } else {
            $update = false;
        }
		...
		$currentTargetResult=$corporateDb->fetchTargetValue('marriott');
        ...
        foreach ($rResult as $aRow) {
            ...
            if($update){
				if($checkDate>=$startDate && $checkDate<=$endDate){
					$row[] = '<a href="/marriott/edit-daily-sales/' . base64_encode($aRow['sales_id']) . '" title="Edit"><i class="fa fa-pencil"> </i></a>';
				}else{
					$row[]='';
				}
    ...
    public function fetchMarriottDailySales($salesId) {
