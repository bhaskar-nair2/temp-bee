...
class TajDailySalesTable extends AbstractTableGateway {

    protected $table = 'taj_daily_sales';

    public function __construct(Adapter $adapter) {
        $this->adapter = $adapter;
    }
    
	public function addTajDailySalesDetails($params) {
        $result = "";
        ...
        if (trim($params['salesDate']) != "") {
			...
			if($corporateId>0){
				...
				$dbAdapter = $this->adapter;
				$tajDb=new TajTable($dbAdapter);
				$tajDb->updateCurrentTarget($corporateId,$params['monthlyTarget']);
			}
			...
			$cQuery = $sql->select()->from('taj_daily_sales')
			...
			foreach($cResult as $val){
				...
				$this->updateTajDailySalesCurrentTarget($val['sales_id'],$todayTarget,$currentTarget,$previousTarget,$corporateId);
				
			}
			
			//event log
			$subject = $lastInsertedId;
			$eventType = 'taj daily sales-add';
			$action = 'added a new taj daily sales with the date '.$params['salesDate'];
			$resourceName = 'Taj';
	...
	public function updateTajDailySalesDetails($params) {
        if (trim($params['salesId'])!="" && trim($params['salesDate']) != "") {
			...
			//event log
			$subject = $lastInsertedId;
			$eventType = 'taj daily sales-update';
			$action = 'updated a taj daily sales with the date '.$params['salesDate'];
			$resourceName = 'Taj';
			$eventLogDb = new EventLogTable($this->adapter);
			$eventLogDb->addEventLog($subject, $eventType, $action, $resourceName);
	...
	public function updateTajDailySalesCurrentTarget($salesId,$todayTarget,$currentTarget,$previousTarget,$corporateId){
		$data = array(
		...
		if($corporateId>0){
			...
			$dbAdapter = $this->adapter;
			$tajDb=new TajTable($dbAdapter);
			$tajDb->updateCurrentTarget($corporateId,$currentTarget);
		}
	}
	
    public function fetchAllTajDailySales($parameters,$acl) {
        /* Array of database columns which should be read and sent back to DataTables. Use a space where
         * you want to insert a non-database field (for example a counter or static image)
        */
		$queryContainer = new Container('query');
        $aColumns = array('DATE_FORMAT(sales_date,"%d-%b-%Y")','suv','sedan','daily_target','monthly_target');
        $orderColumns = array('sales_date','suv','sedan','daily_target','monthly_target');
        ...
        */
        ...
        $dbAdapter = $this->adapter;
        $sql = new Sql($dbAdapter);
        $sQuery = $sql->select()->from('taj_daily_sales');
		...
		$iQuery = $sql->select()->from('taj_daily_sales');
		if(isset($parameters['startDate']) && trim($parameters['startDate'])!="" && isset($parameters['endDate']) && trim($parameters['endDate'])!=""){
		...
		$currentTargetResult=$corporateDb->fetchTargetValue('taj');
        ...
        foreach ($rResult as $aRow) {
            ...
            if($update){
				...
				$row[] = '<a href="/taj/edit-daily-sales/' . base64_encode($aRow['sales_id']) . '" title="Edit"><i class="fa fa-pencil"> </i></a>';
				}else{
					$row[]='';
				}
            }
    ...
    public function fetchTajDailySales($salesId) {
