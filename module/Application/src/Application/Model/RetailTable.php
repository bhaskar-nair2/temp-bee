...
class RetailTable extends AbstractTableGateway {
	...
	public function addRetailTargetDetails($params){
		if (trim($params['targetMonth'])!="") {
			...
			$eventType = 'retail target-add';
			$action = 'added a new retail target with the date '."01-".$params['targetMonth'];
			$resourceName = 'Retail';
	...
	public function updateRetailTargetDetails($params) {
        if (trim($params['retailId'])!="") {
			$commonService=new CommonService();
			$logincontainer = new Container('credo');
			$retailId=base64_decode($params['retailId']);
			$targetMonth="";
			$targetYear="";
			if(isset($params['targetMonth']) && trim($params['targetMonth'])!=""){
				$targetDate=$commonService->dateFormat("01-".$params['targetMonth']);
            ...
            $this->update($data, array('retail_id' => $retailId));
            //event log
			$subject = $retailId;
			$eventType = 'retail target-update';
			$action = 'updated a retail target with the date '.'01-'.$params['targetMonth'];
			$resourceName = 'Retail';
			$eventLogDb = new EventLogTable($this->adapter);
			$eventLogDb->addEventLog($subject, $eventType, $action, $resourceName);
			return $retailId;
    ...
    public function fetchAllRetailTargetDetails($parameters,$acl) {
        ...
        */
        ...
        if ($acl->isAllowed($role, 'Admin\Controller\Retail', 'edit')) {
            $update = true;
        } else {
            $update = false;
        }
        ...
        foreach ($rResult as $aRow) {
			...
			if($update){
				$row[] = '<a href="./edit/' . base64_encode($aRow['retail_id']) . '" title="Edit"><i class="fa fa-pencil"> </i></a>';
			}
            $output['aaData'][] = $row;
        }
        return $output;
	...
	public function fetchRetailTarget($targetId) {
        $row = $this->select(array('retail_id' => (int) $targetId))->current();
        return $row;
    }
	
    public function updateCurrentTarget($corporateId,$currentTarget){
		$this->update(array('current_target'=>$currentTarget), array('retail_id' => $corporateId));
	}
}
?>
